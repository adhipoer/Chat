...
namespace aes
{
 ...
 // Big integer di C# apa?  di java tuh math.BigInteger
	...
	//    static byte[] nounce = {
	//        (byte) 0x15, (byte) 0xF7, (byte) 0x3B, (byte) 0x69,
	//        (byte) 0x6C, (byte) 0x40, (byte) 0x48, (byte) 0x2F,
	//        (byte) 0x5C, (byte) 0xCF, (byte) 0x59, (byte) 0x02,
	//        (byte) 0x66, (byte) 0x48, (byte) 0x2D, (byte) 0xC7
	//    };
	...
	//            System.out.println(Integer.toString(temp[i] & 0xFF, 16));
		...
		internal static sbyte[][] Subtitution(sbyte[][] state)
		{
			...
			for (i = 0; i < 4; i++)
			{
				for (j = 0; j < 4; j++)
				{
					state[i][j] = (sbyte) is_box[state[i][j] & 0xFF];
				}
			}
			return state;
		}
 ...
 // throws di C# make apa ->static String HexStringToString(String string) throws java.io.UnsupportedEncodingException
	...
	//            System.out.println(ByteArrayToHexString(nounce));
				...
				for (j = 0; j < c; j++)
				{
					if (j == 0)
					{
						state = AddRoundKey(state, keyList[c - j - 1]);
					}
					else if (j == c - 1)
					{
	...
	//        System.out.println("e2f3dfda68f5b81a8e7b3b5a741fc0d5c79ecf145a044fb6519ef273cb0da5cc");
	//        String temp = Decrypt("e2f3dfda68f5b81a8e7b3b5a741fc0d5c79ecf145a044fb6519ef273cb0da5cc", initKey, 10);
	//        System.out.println(temp);        
